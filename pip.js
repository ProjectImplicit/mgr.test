define(['pipAPI'], function(APIconstructor) {
    ...
    API.addSettings('base_url', 'https://app-prod-03.implicit.harvard.edu/implicit/user/ezlotnick/mgr.test/');
    API.addSettings('canvas', {canvasBackground:'#fff', css: {color:'black'}});

    API.addSequence([
        {inherit: 'locationTest'},
        {inherit: 'clickTest'},
        {
            mixer: 'repeat',
            times: '10',
            data: [ {inherit:'blinkTest'} ]
        }
    ]);

    API.addTrialSets('locationTest',{
    ...
    API.addTrialSets('blinkTest', {
        input: [ {handle:'finish',on:'timeout', duration:'250'} ],
        layout: [
            { media : 'Does the black box blink?', location:{top:20}, css:{color:'red'} },
